...
# setwd
  setwd("C://Users//larseng//Documents//Manuscript Pilot//TMA")
...
# try this just for the month of February, thus the date:
  ...
  deer.df.feb18b <- deer.df[deer.df$DATE >= as.Date("2017-12-01") & deer.df$DATE <= as.Date("2018-02-28"), ]
...
# cast df
  ...
  # cast dfB
  deer.df.feb18b <- cast(data = deer.df.feb18b, formula = SITE~DATE, mean)
  deer.df.feb18b <- as.matrix.cast_df(deer.df.feb18b[,-1])
  
    
# create unmarked frames with data 
  deer.df.feb18 <- unmarkedFramePCount(y = deer.df.feb18, siteCovs = deer.covars.scaled)
  
  # create unmarked frames with data B
  deer.df.feb18b <- unmarkedFramePCount(y = deer.df.feb18b, siteCovs = deer.covars.scaled)
...
# run model
  ...
  # run model B
  feb18.modb <- pcount(~SLOPE ~ASPECT + ELE, data=deer.df.feb18b, K = 150, se = T,
                      starts = c(0, 0, 0, 0, 0),
                      control = list(trace=T, REPORT = 1))
...
# extract EBUP  
  ...
  # extract EBUP  B
  re.feb18b <- ranef(feb18.modb)
  feb18.EBUPb <- bup(re.feb18b, stat = "mean")
  feb18.CIb <- confint(re.feb18b, level = 0.95)
  feb18.CIb <- as.data.frame(feb18.CIb)
  feb18.ESTb <- data.frame(Estimate = sum(feb18.EBUPb), Lower = sum(feb18.CIb$`2.5%`), Upper = sum(feb18.CIb$`97.5%`))
  FEB18.estb <- feb18.ESTb/7
...
# make figure 2: deer density vs method
  ...
  nmm.dfb <- data.frame(TYPE = 'NMMb', MEAN = FEB18.estb$Estimate, SD = NA, LOWER = FEB18.estb$Lower, UPPER = FEB18.estb$Upper)
  
  fig2.df <- rbind(fl.df, mr.df, nmm.df, nmm.dfb)
  fig2.df$TYPE <- c("Drone\n (Feb)", 'Mark-resight\n (Dec–Feb)', "NMM\n (Jan–Mar)", "NMM\n (Dec–Feb)")
  #fig2.df$TYPE <- factor(fig2.df$TYPE, levels=c('MR', 'NMMb', 'NMM', 'UAI'))
  fig2.df$TYPE <- factor(fig2.df$TYPE, levels=c('Mark-resight\n (Dec–Feb)', "NMM\n (Dec–Feb)", "NMM\n (Jan–Mar)", "Drone\n (Feb)"))
  ...
  plot(fig2.df$ORDER, fig2.df$MEAN, type = 'p', pch = 21, col = 'black', bg = c('grey','black', 'black', 'black'), 
       axes = FALSE, cex = 4, ylim = c(0, 50), xlim = c(0.8, 4.0), ylab = '', xlab = '')
  #plot(fig2.df$ORDER, fig2.df$MEAN, type = 'p', pch = 21, col = 'black', bg = c('grey','black', 'black', 'black'), 
  ...
  axis(side = 1, at = as.numeric(fig2.df$TYPE), labels = fig2.df$TYPE, cex.axis=2, line = 0, padj = 1)
...
# make figure 4: deer density vs NDVI
  ...
  arrows(x0 = 7, x1 = 7, y0 = 31.3, y1 = 39, angle = 90, length = 0.05, lwd = 3)  
  arrows(x0 = 7, x1 = 7, y0 = 31.3, y1 = 23.7, angle = 90, length = 0.05, lwd = 3) 
